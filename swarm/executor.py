...
职责：
    1. 向子节点注册（上报自身能力）
    2. 接收子节点的自然语言任务指令
    3. 调用本机 AeroWeaver 的 AgentLoop 执行任务
    4. 执行完成后生成报告上报子节点

关键：Executor 不需要改动现有 AeroWeaver 代码。
...
class Executor:
    ...
    包装现有 AeroWeaver 的 AgentLoop，接入集群通信。
    ...
    def __init__(self, port: int = 6200, coordinator_url: str = "http://localhost:6100",
        ...
        self.agent_loop = agent_loop  # 现有 AeroWeaver AgentLoop
