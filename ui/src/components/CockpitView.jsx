...
function PiPCamera({ image, imageUrl, label }) {
  return (
    <div style={{ width: 130, height: 75, borderRadius: 6, overflow: 'hidden', border: '1px solid ' + GLASS_BORDER, position: 'relative', background: '#111', flexShrink: 0 }}>
      {(image || imageUrl) && <img src={image ? 'data:image/jpeg;base64,' + image : imageUrl} alt={label} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
...
export default function CockpitView({
  ...
  const [pixelStatus, setPixelStatus] = useState('switching');
  const [controlStatus, setControlStatus] = useState({ ok: null, message: '' });
  ...
  const sendStop = useCallback(() => {
    if (socket) socket.emit('velocity_control', {
      robot_id: robotId,
      forward: 0,
      right: 0,
      down: 0,
  ...
  const emitVelocity = useCallback((keys) => {
    if (!socket || !keys?.size) return;
    socket.emit('velocity_control', {
      robot_id: robotId,
      ...buildCmd(keys),
    });
  }, [socket, robotId, buildCmd]);

  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.key === 'Escape') { sendStop(); onClose(); return; }
      // Number keys switch camera views: 1=front 2=rear 3=left 4=right 5=down
  ...
  useEffect(() => {
    sendIntervalRef.current = setInterval(() => {
      emitVelocity(keysRef.current);
    }, SEND_INTERVAL);
    return () => { clearInterval(sendIntervalRef.current); sendStop(); };
  }, [emitVelocity, sendStop]);

  useEffect(() => {
    if (!socket) return;
    const handler = (data) => setTelemetry(data);
    socket.on('telemetry', handler);
    const poll = setInterval(() => socket.emit('get_telemetry', { robot_id: robotId }), TELEMETRY_POLL);
    socket.emit('get_telemetry', { robot_id: robotId });
  ...
  useEffect(() => {
    if (!socket) return;
    const handler = (data) => {
      if (data?.robot_id && data.robot_id !== robotId) return;
      setControlStatus({
        ok: Boolean(data?.ok),
        message: data?.msg || data?.error || '',
      });
    };
    socket.on('velocity_result', handler);
    return () => socket.off('velocity_result', handler);
  }, [socket, robotId]);

  useEffect(() => {
    const controller = new AbortController();
    setPixelStatus('switching');
    fetch(`${API_BASE}/api/pixel-streaming/view`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ robot_id: robotId, view: activeView }),
  ...
  useEffect(() => {
    ...
    const fetchLidar = () => {
      fetch(`${API_BASE}/api/sensor/lidar?robot_id=${encodeURIComponent(robotId)}`)
        ...
        .then((data) => {
          if (active && data && !data.error) setSelectedLidar(data);
        })
        .catch(() => {})
        .finally(() => {
          if (active) poll = setTimeout(fetchLidar, 1500);
        });
    };
    fetchLidar();
  ...
  useEffect(() => {
    ...
    const lidar = selectedLidar || sensorLidar;
    if (!canvas || !lidar) return;
    const ctx = canvas.getContext('2d');
    const w = canvas.width, h = canvas.height, cx = w / 2, cy = h / 2;
    const maxRange = lidar.range_max || 10;
    ...
    const angleInc = lidar.angle_increment || 0;
    const rangeMin = lidar.range_min || 0;
  ...
  const mainImgUrl = cameraUrl(activeView);

  // PiP: show the other camera views
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: '#000', cursor: 'crosshair', fontFamily: 'monospace', overflow: 'hidden' }}>
      ...
      {pixelStatus !== 'fallback' && (
        <iframe
          src={pixelStreamingUrl}
          title={`${uavLabel} ${viewLabels[activeView]} WebRTC`}
          allow="autoplay; fullscreen"
          style={{
            position: 'absolute', inset: 0, width: '100%', height: '100%',
            border: 0, background: '#000', pointerEvents: 'none',
          }}
        />
      )}
      {pixelStatus === 'fallback' && (mainImg || mainImgUrl) && (
        <img src={mainImgUrl || 'data:image/jpeg;base64,' + mainImg} alt={activeView}
          style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
      )}
      {pixelStatus === 'fallback' && !mainImg && !mainImgUrl && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center',
          color: 'rgba(0,212,255,0.3)', fontSize: 16, fontFamily: 'monospace' }}>
          {t('无信号', 'NO SIGNAL')} — {viewLabels[activeView]}
        </div>
      )}
      ...
      <div style={{ position: 'absolute', top: 52, left: '50%', transform: 'translateX(-50%)', zIndex: 4,
        ...
        {uavLabel} · {viewLabels[activeView]} · {pixelStatus === 'playing' ? 'WEBRTC' : pixelStatus === 'fallback' ? 'MJPEG' : t('切换中', 'SWITCHING')}
      </div>
      ...
      <div style={{
        ...
        <span style={{ background: BADGE_BG, borderRadius: 6, padding: '4px 10px', fontFamily: 'monospace', fontSize: 13, color: '#fff' }}>
          {telemetry.armed ? t('🟢 已解锁', '🟢 ARMED') : t('🔴 未解锁', '🔴 DISARM')}
        </span>
        <span style={{ background: BADGE_BG, borderRadius: 6, padding: '4px 10px', fontFamily: 'monospace', fontSize: 13, color: '#fff' }}>
          {telemetry.in_air ? t('✈ 飞行', '✈ FLIGHT') : t('🅿 地面', '🅿 GND')}
        </span>
        <span style={{ background: BADGE_BG, borderRadius: 6, padding: '4px 10px', fontFamily: 'monospace', fontSize: 13, color: '#fff' }}>
          🔋 {battPct}%
        </span>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 6, pointerEvents: 'auto' }}>
          {pipViews.map(v => (
            <div key={v} onClick={() => setActiveView(v)} style={{ cursor: 'pointer' }}>
              <PiPCamera image={viewImages[v]} imageUrl={cameraSnapshotUrl(v)} label={viewLabels[v]} />
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, pointerEvents: 'auto' }}>
          ...
          <div style={{ fontSize: 10, color: 'rgba(0,212,255,0.5)', pointerEvents: 'none', textAlign: 'center', whiteSpace: 'nowrap' }}>
            {t('W/A/S/D 移动 · SPACE 上升 · SHIFT 下降 · Q/E 旋转 · 1-5 切换视角 · ESC 退出', 'W/A/S/D Move · SPACE Up · SHIFT Down · Q/E Rotate · 1-5 Switch View · ESC Exit')}
          </div>
          <div style={{
            minHeight: 14,
            maxWidth: 420,
            ...
            whiteSpace: 'nowrap',
            fontSize: 10,
            color: controlStatus.ok === false ? '#ff5c5c' : controlStatus.ok === true ? '#67e86f' : 'rgba(0,212,255,0.5)',
            pointerEvents: 'none',
          }}>
            {controlStatus.message || t('等待驾驶指令', 'READY FOR CONTROL')}
          </div>
        </div>
