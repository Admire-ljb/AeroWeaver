#!/usr/bin/env bash
# ============================================================
# AeroWeaver — PX4/Gazebo guided setup doctor
# ============================================================
...
# is configured with AeroWeaver's modified UAV model and explains the next command
...
printf "============================================================\n"
printf " AeroWeaver PX4/Gazebo Doctor\n"
printf " Project: %s\n" "$PROJECT_DIR"
printf " PX4_DIR: %s\n" "$PX4_DIR"
printf " World:   %s\n" "$WORLD"
printf " Model:   %s\n" "$MODEL"
...
if [ "$MODEL" != "$SHOWCASE_MODEL" ]; then
  warn "Current model '${MODEL}' is not the AeroWeaver modified UAV model. Use '${SHOWCASE_MODEL}' for demos and artifact checks."
fi
...
if [ "$MODEL" = "$SHOWCASE_MODEL" ] && [ -f "$CUSTOM_MODEL_DIR/model.sdf" ]; then
  for required_sensor in cam_front cam_rear cam_left cam_right cam_down lidar_2d; do
    if grep -q "$required_sensor" "$CUSTOM_MODEL_DIR/model.sdf"; then
      pass "AeroWeaver model defines sensor: ${required_sensor}"
    else
      fail "AeroWeaver model is missing required sensor in repository SDF: ${required_sensor}"
    fi
  done
fi
...
if [ -d "$LOCAL_MODEL_DIR/$MODEL" ] || [ -d "$PX4_MODEL_DIR/$MODEL" ]; then
  ...
  if [ "$MODEL" = "$SHOWCASE_MODEL" ]; then
    fail "AeroWeaver modified UAV model '${MODEL}' is not installed. Run ./scripts/setup_px4.sh"
  elif [ "$MODEL" = "x500" ]; then
