...
def main():
    ...
    remote = "/home/runner/.aeroweaver-experiments/memory-collection/" + stamp
    ...
    try:
        ...
        command = "/home/runner/.venvs/aeroweaver/bin/python -u " + shlex.quote(remote + "/run.py")
        command += " --output " + shlex.quote(remote + "/outputs") + " 2>&1"
        (local / "launch.json").write_text(json.dumps({"host": "192.0.2.10", "command": command,
            ...
            if report_path.exists():
                ...
                for item in report["started_missions"]:
                    ...
                    for name in ("summary.json", "experience.jsonl", "trace.jsonl", "reward_manifest.json"):
                        try:
                            sftp.get("/home/runner/AeroWeaver/results/mock-tasks/" + mission + "/" + name,
                                     str(directory / name))
                        except FileNotFoundError:
