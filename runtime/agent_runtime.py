...
class AgentRuntime:
    ...
    def dispatch_skill(self, step: dict) -> ExecutionResult:
        ...
        # Bind the adapter per execution thread so concurrent UAV skills do not
        # race on one shared "active vehicle" field.
        from adapters.adapter_manager import robot_adapter_context
        with robot_adapter_context(robot_id):
