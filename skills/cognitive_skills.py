...
# 工作目录基准（限制文件读写范围）
_WORK_DIR = Path(os.environ.get("AEROWEAVER_WORKDIR", ".")).resolve()
...
_MAX_PYTHON_CODE_LEN = 4_000
_MAX_PYTHON_AST_NODES = 500
...
_SAFE_MATH_NAMES = {
    ...
    for name in (
        ...
        "exp", "fabs", "floor", "hypot", "log", "log10", "pi", "radians",
        "sin", "sqrt", "tan", "tau",
    )
}
_ALLOWED_PYTHON_NODES = (
    ast.Module, ast.Expr, ast.Assign, ast.AugAssign, ast.If, ast.For, ast.While,
    ...
    ast.Slice, ast.BinOp, ast.UnaryOp, ast.BoolOp, ast.Compare, ast.Call,
...
def _validate_python(code: str) -> ast.Module:
    if len(code) > _MAX_PYTHON_CODE_LEN:
        raise ValueError(f"code exceeds {_MAX_PYTHON_CODE_LEN} characters")
    tree = ast.parse(code, mode="exec")
    nodes = list(ast.walk(tree))
    if len(nodes) > _MAX_PYTHON_AST_NODES:
        raise ValueError(f"code exceeds {_MAX_PYTHON_AST_NODES} syntax nodes")
    allowed_calls = set(_SAFE_PYTHON_BUILTINS) | set(_SAFE_MATH_NAMES)
    for node in nodes:
        ...
        if isinstance(node, ast.Call):
            if not isinstance(node.func, ast.Name) or node.func.id not in allowed_calls:
                raise ValueError("only approved calculation functions may be called")
    return tree


def _run_python_worker(code: str, output_queue) -> None:
    stdout = io.StringIO()
    stderr = io.StringIO()
    globals_dict = {
    ...
    try:
        ...
        with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            exec(compile(tree, "<skill-run-python>", "exec"), globals_dict, {})
        output_queue.put({
...
class RunPython(Skill):
    ...
    def execute(self, input_data: dict) -> SkillResult:
        ...
        try:
            ...
            return SkillResult(success=False, error_msg=f"代码校验失败: {exc}")

        start = time.time()
        context = multiprocessing.get_context("spawn")
        output_queue = context.Queue(maxsize=1)
        process = context.Process(
            target=_run_python_worker,
        ...
        try:
            ...
            result = {
                "stdout": "",
                "stderr": "Execution worker exited without a result",
        ...
        return SkillResult(
            success=success,
            output=result,
            error_msg="" if success else result["stderr"],
            cost_time=elapsed,
            logs=[f"run_python: exit={result['exit_code']} ({elapsed}s)"],
        )
