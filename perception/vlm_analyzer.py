...
class VLMAnalyzer:
    ...
    def __init__(
        ...
        self._timeout = max(
            2,
            int(timeout if timeout is not None else os.environ.get("VLM_TIMEOUT", "8")),
        )
        self._max_retries = max(1, int(os.environ.get("VLM_MAX_RETRIES", "1")))
    ...
    def analyze_image(
        ...
        if time.monotonic() < self._unavailable_until:
            logger.debug("VLM circuit breaker active: %s", self._last_error)
            return None
        if "api.openai.com" in self._base_url and not self._api_key:
            self._last_error = "VLM_API_KEY is not configured"
            self._unavailable_until = time.monotonic() + 300.0
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "available": time.monotonic() >= self._unavailable_until,
        }
