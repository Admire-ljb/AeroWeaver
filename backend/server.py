...
def _send_agent_network_message(source, target, content, sid, *, mission_id="", kind="peer", inject=True):
    ...
    if kind in {"peer", "result"}:
        ...
        # Initial connection (including reconnect) restores the history snapshot.
        # Sending it for every message queues stale state during multi-agent runs.
        # Stream only the new event here.
    if inject:
