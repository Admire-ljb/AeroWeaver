...
class Takeoff(Skill):
    ...
    def execute(self, input_data: dict) -> SkillResult:
        ...
        altitude = min(max(float(input_data.get("altitude", 5.0)), 0.5), 100.0)
        adapter = _get_adapter()
        if adapter is None:
        ...
        return SkillResult(
            ...
            output={
                "actual_altitude": result.data.get("altitude"),
                "relative_climb": result.data.get("climb_m", altitude),
...
class FlyTo(Skill):
    name = "fly_to"
    description = "飞到指定 AirSim 世界坐标；自动使用机腹测距保持安全离地高度，禁止钻入地面。"
    skill_type = "hard"
    robot_type = ["UAV"]
    preconditions = []
    cost = 3.0
    input_schema = {
        "target_position": "[x, y, z] AirSim 世界坐标。地图取点会继承当前 z，底层会按局部地形自动抬高不安全的 z。",
    ...
    def execute(self, input_data: dict) -> SkillResult:
        ...
        effective_target = result.data.get("effective_target", [n, e, d])
        effective_d = float(effective_target[2])
        err = math.sqrt(
            (final_pos.north - n)**2
            + (final_pos.east - e)**2
            + (final_pos.down - effective_d)**2
        )
        logger.info(
        ...
        return SkillResult(
            ...
            output={
                ...
                "requested_position": [round(n, 1), round(e, 1), round(d, 1)],
                "distance_traveled": round(dist, 1),
                "altitude": round(final_alt, 1),
                "ground_clearance": result.data.get("ground_clearance"),
                "target_adjusted": result.data.get("target_adjusted", False),
...
class ChangeAltitude(Skill):
    ...
    def execute(self, input_data: dict) -> SkillResult:
        altitude = min(max(float(input_data.get("altitude", 10.0)), 5.0), 120.0)
        ...
        t0 = time.time()
        if hasattr(adapter, "change_altitude"):
            logger.info(f"[ChangeAltitude] using adapter.change_altitude(target={altitude:.1f})")
            result = adapter.change_altitude(altitude, speed=5.0)
        else:
            ...
            result = adapter.fly_to_ned(pos.north, pos.east, target_z, speed=5.0)
        dt = round(time.time() - t0, 2)
        ...
        return SkillResult(
            ...
            output={
                ...
                "current_position": [round(final_pos.north, 1), round(final_pos.east, 1), round(final_pos.down, 1)],
                "minimum_applied": bool(getattr(result, "data", {}).get("minimum_applied", False)),
            },
            cost_time=dt,
...
class GetBattery(Skill):
    ...
    def execute(self, input_data: dict) -> SkillResult:
        ...
        dt = round(time.time() - t0, 3)
        is_airsim = "airsim" in str(getattr(adapter, "name", "")).lower()
        ...
        return SkillResult(
            ...
            output={
                "voltage_v": round(v, 2),
                "remaining_percent": round(pct * 100.0, 1),
                "remaining_fraction": round(pct, 3),
                "source": "airsim_simulated_unlimited" if is_airsim else adapter.name,
            },
            cost_time=dt,
...
class ReturnToLaunch(Skill):
    ...
    def execute(self, input_data: dict) -> SkillResult:
        ...
        return SkillResult(
            success=result.success,
            output={"rtl_time": dt, **dict(getattr(result, "data", {}) or {})},
            error_msg=result.message if not result.success else "",
            cost_time=dt,
...
class FlyRelative(Skill):
    ...
    def execute(self, input_data: dict) -> SkillResult:
        ...
        speed = min(max(float(input_data.get("speed", 5.0)), 0.5), 30.0)
...
class LookAround(Skill):
    ...
    def execute(self, input_data: dict) -> SkillResult:
        ...
        duration = min(max(float(input_data.get("duration", 8)), 0.5), 60.0)
        state = adapter.get_state()
        ...
        try:
            if hasattr(adapter, "rotate_by"):
                rotate_result = adapter.rotate_by(360.0, duration=duration)
                if not rotate_result.success:
                    raise RuntimeError(rotate_result.message)
            else:
                while time.time() - start_t < duration:
                    if hasattr(adapter, 'name') and 'mavsdk' in adapter.name.lower():
                        ...
                        time.sleep(0.2)
                if hasattr(adapter, 'name') and 'mavsdk' in adapter.name.lower():
                    adapter.set_velocity_body(0, 0, 0, duration=0.2, yaw_rate=0)
                else:
...
class MarkLocation(Skill):
    ...
    def execute(self, input_data: dict) -> SkillResult:
        ...
        mark = {
            ...
            "robot_id": (
                adapter.get_active_robot()
                if hasattr(adapter, "get_active_robot")
                else None
            ),
            "label": label,
            "position": ned,
...
class GetMarks(Skill):
    ...
    def execute(self, input_data: dict) -> SkillResult:
        ...
        robot_id = (
            adapter.get_active_robot()
            if adapter is not None and hasattr(adapter, "get_active_robot")
            else None
        )
        marks = [
            mark.copy()
            for mark in MarkLocation._marks
            if robot_id is None or mark.get("robot_id") in (None, robot_id)
        ]
        return SkillResult(
            success=True,
...
class OrbitInspect(Skill):
    ...
    def execute(self, input_data: dict) -> SkillResult:
        ...
        radius = min(max(float(input_data.get("radius", 25)), 2.0), 500.0)
        start_h = min(max(float(input_data.get("start_height", 20)), 5.0), 120.0)
        end_h = min(max(float(input_data.get("end_height", 80)), start_h), 120.0)
        h_step = min(max(float(input_data.get("height_step", 15)), 1.0), 60.0)
        pts_per_layer = min(max(int(input_data.get("points_per_layer", 8)), 3), 24)
        speed = min(max(float(input_data.get("speed", 5.0)), 0.5), 30.0)
        ...
        for layer_idx, height in enumerate(layers):
            ...
            for pt_idx in range(pts_per_layer):
                ...
                clearance = (
                    adapter.get_ground_clearance()
                    if hasattr(adapter, "get_ground_clearance")
                    else None
                )
                target_down = (
                    current_pos.down + clearance - height
                ...
                if hasattr(adapter, "change_altitude"):
                    altitude_result = adapter.change_altitude(height, speed=speed)
                    if not altitude_result.success:
                        logs.append(
                            f"⚠️ 航点({wp_n:.0f},{wp_e:.0f})高度校准失败: "
                            f"{altitude_result.message}"
                        )
                        continue
