...
class Executor:
    ...
    def execute_skill(
        ...
        # Mock adapter 是单例；执行前绑定目标 UAV，避免不同 UAV 共享同一套位置状态。
        try:
            from adapters.adapter_manager import get_adapter
            adapter = get_adapter()
            set_active_robot = getattr(adapter, "set_active_robot", None) if adapter is not None else None
        ...
        # ── 6. 更新机器人状态为 idle，并同步执行后的遥测 ─────────────────────
        new_status = "idle" if skill_result.success else "error"
        robot_update = {"status": new_status}
        try:
            from adapters.adapter_manager import get_adapter
            adapter = get_adapter()
            is_connected = False
            if adapter is not None:
                connected_attr = getattr(adapter, "is_connected", None)
                is_connected = connected_attr() if callable(connected_attr) else bool(connected_attr)
            if adapter is not None and is_connected:
                pos = adapter.get_position()
                if pos is not None:
                    robot_update["position"] = [
                        round(pos.north, 2),
                        round(pos.east, 2),
                        round(pos.down, 2),
                    ]
                get_battery = getattr(adapter, "get_battery", None)
                if callable(get_battery):
                    bat = get_battery()
                    if bat:
                        raw = bat[1]
                        robot_update["battery"] = round(min(raw, 100.0) if raw > 1 else raw * 100.0, 1)
                is_in_air = getattr(adapter, "is_in_air", None)
                if callable(is_in_air):
                    robot_update["in_air"] = is_in_air()
        except Exception:
            pass
        self._world_model.update_world_state({
            "robots": {robot_id: robot_update}
        })
